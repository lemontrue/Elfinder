...
can.Construct('PR', {
    ...
    showLetterSent: function () {
        this.el.html(can.view('/Assets/views/pass/letter_sent.hbs', {
            event: {
                /**
    ...
    notRecovery: function () {
        this.el.html(can.view('/Assets/views/pass/link_not_active.hbs', {}));
    }
});

PR.actions.enterSMS = can.Control({
    view: '/Assets/views/pass/enter_sms.hbs'
}, {
    ...
    showLetterSent: function () {
        this.element.html(can.view('/Assets/views/pass/letter_sent_sms.hbs', {
            event: {}
        }));
    }
});
...
PR.actions.enterPhone = can.Control({
    view: '/Assets/views/pass/enter_phone.hbs'
}, {
    init: function (el, options) {
...
PR.actions.enterEmail = can.Control({
    view: '/Assets/views/pass/enter_email.hbs'
}, {
    init: function (el, options) {
    ...
    timeProtect: function() {
        ...
        if (diff > 30*60*1000) {
            this.error({
                message: 'Время ожидания подошло к концу. Пройдите процедуру восстановления пароля с начала'
            });
            return false
        }
...
PR.actions.captcha = can.Control({
    view: '/Assets/views/pass/captcha.hbs'
}, {
    init: function (el, options) {
        this.error = can.compute(false);
        this.codeValue = can.compute('');
