var SignAndEncrypt = can.Control({
    ...
    init: function(el, options) {
        ...
        try {
            ...
            var view = can.view('/Scripts/master/view/activity/sign_and_encrypt.hbs', {
                ...
                event: {
                    refiles: delegate(this.main.reFiles, this.main, [true, 'Вы уверены, что хотите изменить действия над файлами? Все не сохраненные данные буду потеряны.']),
                    ...
                    delete: {
                        ...
                        personal: delegate(function(scope, el, ev) {
                            console.log(this.certificate.personal);
                            var s = null;
    ...
    makeCertificate: function() {
        try {
            ...
            new Logger([
                'Перестройка шаблонов - (Шифрования - %a : Подписи - %b)', {
    ...
    run: function(need, pin, save) {
        ...
        var ajaxObj,
            processID = this.scope.iprocess.add((this.operationCode() === 1) ? 'Выполняется подпись файлов...' : ((this.operationCode() === 2) ? 'Выполняется шифрование файлов' : 'Выполняется подпись и шифрование файлов...'), delegate(function() {
                ajaxObj.abort();
            }, this));
