var Template = can.Construct.extend('Core.Templates.Template', {
    ...
    __getSettings: function() {
        ...
        this.scope.req.begin('user_profile', { profileId: this.ID }, function(req) {
            if (req.status === 'error') {
                new Logger([
                    'При загрузке настроек профиля "%name" возникли проблемы', {
                        '%name': this.Name
                    }
                ], 'error', req);
                return false;
    ...
    __runConfig: function(conf) {
        ...
        if (_signature === undefined) {
            new Logger([
                'Возникла ошибка при получении настроек для подписи, шаблона "%name"', {
                    '%name': this.Name
                }
            ], 'error');
        }
        ...
        if (_encryption === undefined) {
            new Logger([
                'Возникла ошибка при получении настроек для шифрования, шаблона "%name"', {
                    '%name': this.Name
                }
            ], 'error');
        }
